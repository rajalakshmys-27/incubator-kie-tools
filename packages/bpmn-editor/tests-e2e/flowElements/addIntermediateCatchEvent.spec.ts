...
test.describe("Add node - Intermediate Catch Event", () => {
  ...
  test.describe("Intermediate Catch Event operations", () => {
    ...
    //   test("should move intermediate catch event to new position", async ({ palette, page }) => {
    //     await palette.dragNewNode({ type: NodeType.INTERMEDIATE_CATCH_EVENT, targetPosition: { x: 300, y: 300 } });

    //     const catchEvent = page.locator(".kie-bpmn-editor--intermediate-catch-event-node").first();
    //     const boxBefore = await catchEvent.boundingBox();

    //     // Move intermediate catch event to new position
    //     await catchEvent.dragTo(catchEvent, {
    ...
    //   test("should copy and paste intermediate catch event", async ({ palette, jsonModel, page }) => {
    //     await palette.dragNewNode({ type: NodeType.INTERMEDIATE_CATCH_EVENT, targetPosition: { x: 300, y: 300 } });

    //     const catchEvent = page.locator(".kie-bpmn-editor--intermediate-catch-event-node").first();
    //     await catchEvent.click();

    //     // Copy the intermediate catch event
    //     await page.keyboard.press("ControlOrMeta+C");

    //     // Paste the intermediate catch event
    ...
    //     // Both should be intermediate catch events
    //     expect(flowElements.flowElement?.[0].__$$element).toBe("intermediateCatchEvent");
    //     expect(flowElements.flowElement?.[1].__$$element).toBe("intermediateCatchEvent");
    //   });
  });
});
