...
test.describe("Add node - Start Event", () => {
  ...
  test.describe("Start Event operations", () => {
    ...
    //     // Copy the start event
    //     await page.keyboard.press("ControlOrMeta+C");

    //     // Paste the start event
    //     await page.keyboard.press("ControlOrMeta+V");

    //     // Verify both start events exist in JSON model
    //     const flowElements = await jsonModel.getProcess();
    //     expect(flowElements.flowElement?.length).toBe(2);

    //     // Both should be start events
    //     expect(flowElements.flowElement?.[0].__$$element).toBe("startEvent");
    //     expect(flowElements.flowElement?.[1].__$$element).toBe("startEvent");
    //   });
  });
});
